...
const { chromium } = require(process.env.PLAYWRIGHT_MODULE || 'C:/Users/haing/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules/playwright');
...
(async () => {
  ...
  fs.writeFileSync(config,JSON.stringify({workspace_root:project,state_dir:path.join(temp,'state'),tasks:{git_status:['git','status','--short']}}));
  const child=spawn('uv',['run','--with','mcp==1.30.0','--python','3.13',path.join(__dirname,'server.py'),'--config',config],{windowsHide:true,stdio:['pipe','pipe','pipe']});
  ...
  async function call(name,args={}){const r=await rpc('tools/call',{name,arguments:args});assert.ok(!r.isError,JSON.stringify(r.content));return r.structuredContent;}
  try {
    await rpc('initialize',{protocolVersion:'2025-11-25',capabilities:{},clientInfo:{name:'bridge-ui-verification',version:'1'}});notify('notifications/initialized');
    const schema=await rpc('tools/list');assert.equal(schema.tools.length,25);
    const info=await call('project_info');assert.equal(info.workspace_root.replaceAll('\\','/'),project.replaceAll('\\','/'));
    const malicious='<img src=x onerror="window.PWNED=true">\r\nKhầy\r\n';
    const change=await call('prepare_changes',{title:'Kiểm thử UI',edits:[{path:'xin chao.txt',content:malicious}],request_id:'ui-test-prepare'});
    assert.ok(!fs.existsSync(path.join(project,'xin chao.txt')));
    const resource=await rpc('resources/read',{uri:'ui://local-bridge/control-panel-v1.html'});
    ...
    await page.exposeFunction('mcpCall',async message=>rpc(message.method,message.params));
    ...
    assert.ok(!fs.existsSync(path.join(project,'xin chao.txt')));
    assert.equal((await call('apply_changes',{change_id:change.change_id,request_id:`ui-apply_changes-${change.change_id}`})).status,'undone');
