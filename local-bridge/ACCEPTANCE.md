...
| F — không mở lại bảng khi đọc dữ liệu, subscription/resource update khi task hoặc scan đổi trạng thái | **Đạt local wire/UI:** `bridge://task/{run_id}`, `bridge://scan/{scan_id}` và `bridge://change/{change_id}` dùng `subscriptions/listen` + `resources/read`; client cũ tự fallback polling 2 giây, reconnect đọc lại authoritative state. `test_tasks_mcp.cjs`, `test_security_ui.cjs` đo event path và fallback path; ChatGPT Web live vẫn cần restart tunnel/làm mới resource để xác nhận host proxy subscription. |
